...
function IPromise() {
    ...
    var Promise = function Promise(id, context, byforce) {
        ...
        function setStatus(is_fulfilled, newValue) {
            if (_future_value_ !== _waiting) {
                return promise;
            }
            _future_value_ = is_fulfilled ? newValue : (_error.data = newValue, _error);
            return invoke(is_fulfilled ? _sxs : _err, null);
        }
        promise = {
            constructor: Promise,
            ...
            then: function(sxs, fail, context) {
                ...
                if (promise.status() === 'waiting') {
                    return promise;
                }
                return invoke(promise.status() === 'breaked' ? _err : _sxs, null);
            },
            fail: function(fail, context) {
                return promise.then(null, fail, context);
            },
            done: function(sxs, context) {
                return promise.then(sxs, null, context);
            },
            always: function(always, context) {
                return promise.then(always, always, context);
            },
            setTimeout: function(time, errorData) {
            ...
            function next() {
                ...
                setTimeout(function() {
                    ...
                    if (invoke.index < callbacks.length) {
                        invoke.lastValue && invoke.lastValue.constructor && invoke.lastValue.constructor.name === 'Promise' ? invoke.lastValue.always(next) : next();
                    } else {
                        invoke.index = -1;
                        _sxs.length = 0;
                        _err.length = 0;
    ...
    Promise.when = function(id, promises, context) {
        promises = promises || [];
        var values = Promise.argsArray(null);
        var was_error = false;
        var whenPromise = Promise(id, context, null);
        if (!promises.length) {
            return whenPromise.$fulfill(values);
        }
        var advanceWhenState = function(promise, value) {
            if (promise.status() === 'breaked') {
                was_error = true
            }
            values.push(value);
